...
def top_feature_net(input, anchors, inds_inside, num_bases):
    ...
    return stride, feature, scores, probs, deltas, rois, roi_scores # why returning stride??
...
#------------------------------------------------------------------------------
def fusion_net_v2(feature_list, num_class, out_shape=(8,3)):
    # pdb.set_trace()
    num=1 # len(feature_list)
    ...
    with tf.variable_scope('fuse-input') as scope:
        ...
        roi         = feature_list[1]
        pool_height = feature_list[2]
        pool_width  = feature_list[3]
        pool_scale  = feature_list[4]
        # if (pool_height==0 or pool_width==0): continue
        
        roi_features,  roi_idxs = tf_roipooling(feature,roi, pool_height, pool_width, pool_scale, name='{}/pool'.format(0))
...
def fusion_net(feature_list, num_class, out_shape=(8,3)):
    ...
    with tf.variable_scope('fuse-input') as scope:
        for n in range(num):
            ...
            if (pool_height==0 or pool_width==0): continue
            pdb.set_trace()
            roi_features,  roi_idxs = tf_roipooling(feature,roi, pool_height, pool_width, pool_scale, name='{}/pool'.format(n))
